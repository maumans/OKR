...
function KrProgressRow({ kr, krIdx, seuils, objectifId, onAddTask, onViewTask, auth, objCollabId }) {
...
function ObjectifCard({ obj, seuils, seuilPrime, onEdit, onDelete, onAddTask, onViewTask, auth }) {
 ...
 {Number(obj.prime) > 0 && <> · Prime si ≥ {seuilPrime}% : {formatCurrency(obj.prime, devise)}</>}
 </span>
 {tachesC > 0 && (
 ...
 <KrProgressRow key={kr.id} kr={kr} krIdx={i} seuils={seuils} objectifId={obj.id} onAddTask={onAddTask} onViewTask={onViewTask} auth={auth} objCollabId={obj.collaborateur_id} />
 ))}
 </div>
 </motion.div>
 )}
...
export default function IndividuelsIndex({
 ...
 <p className="text-xl font-black text-amber-600 dark:text-amber-400">{formatCurrency(primeTotale, devise)}</p>
 ...
 {objectifs.length > 0 ? (
 objectifs.map(obj => (
 <ObjectifCard key={obj.id} obj={obj} seuils={seuils} seuilPrime={seuilPrime} auth={auth}
