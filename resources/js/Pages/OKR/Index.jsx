...
function CreateObjectifModal({ open, onClose, periodes, defaultCollaborateurId, collaborateurs }) {
    const [error, setError] = useState('');
    const [submitting, setSubmitting] = useState(false);
    const [formData, setFormData] = useState({
        titre: '',
        periode_id: String(periodes[0]?.id || ''),
                    ...
                    <div className="p-5 pb-4">
                        ...
                        <div className="mt-4 space-y-3">
                            ...
                            <div>
                                ...
                                <label className="text-[10px] font-bold text-emerald-500 uppercase tracking-wider">Période</label>
                                <select
                                    value={formData.periode_id}
                                    onChange={e => setField('periode_id', e.target.value)}
                                    className="w-full mt-1 px-3 py-2 bg-white dark:bg-dark-800 border border-gray-200 dark:border-dark-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 transition-all appearance-none cursor-pointer"
                                >
                                    {periodes.map(p => <option key={p.id} value={String(p.id)}>{p.nom}</option>)}
                                </select>
                            </div>
...
function AddTaskModal({ open, onClose, objectifId, resultatsCles = [], defaultResultatCleId, collaborateurs, defaultCollaborateurId }) {
...
function EditObjectifModal({ open, onClose, objectif, collaborateurs, periodes, axes, typesObjectifs = [], typesResultatsCles = [], configuration }) {
    const [error, setError] = useState('');
    const [submitting, setSubmitting] = useState(false);
    ...
    useEffect(() => {
        if (open && objectif) {
            setFormData({
                ...
                periode_id: String(objectif.periode_id || ''),
    ...
    const addKR = () => setField('resultats_cles', [...formData.resultats_cles, { id: null, description: '', type_resultat_cle_id: '', valeur_cible: 100, poids: 1, unite: '' }]);
    const removeKR = (i) => {
    ...
    return (
        <Dialog open={open} onOpenChange={(v) => { if (!v) onClose(); }}>
            <DialogContent aria-describedby={undefined} className="max-w-2xl p-0 overflow-hidden max-h-[90vh]">
                <form onSubmit={handleSubmit}>
                    <div className="p-5 pb-3 border-b border-gray-100 dark:border-dark-700">
                        <DialogHeader>
                            <DialogTitle className="flex items-center gap-2 text-base">
                    ...
                    <div className="p-5 space-y-4 overflow-y-auto max-h-[60vh]">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Période</label>
                                <select value={formData.periode_id || ''} onChange={e => setField('periode_id', e.target.value)}
                                    className="w-full mt-1 px-2.5 py-2 bg-white dark:bg-dark-800 border border-gray-200 dark:border-dark-700 rounded-lg text-xs appearance-none cursor-pointer">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Prime (GNF)</label>
                                ...
                                {(formData.resultats_cles || []).map((kr, i) => (
                                    <div key={kr.id || `new-${i}`} className="p-3 rounded-lg border border-gray-100 dark:border-dark-700 bg-gray-50/50 dark:bg-dark-800/50 space-y-2">
                                        <div className="flex items-start gap-2">
                                            ...
                                            <input type="text" value={kr.description} onChange={e => updateKR(i, 'description', e.target.value)} placeholder="Description du KR..."
                                                className="flex-1 px-2.5 py-1.5 bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-primary-500/30" />
...
function TaskDetailPanel({ tache, onClose, objectifTitre, collaborateurs = [] }) {
...
function KRRow({ kr, krIdx, seuils, onAddTaskForKr, onViewTask, objectifId, defaultExpanded = false }) {
...
function ObjectifCard({ obj, seuils, handleDelete, defaultExpanded = false, onAddTask, onViewTask, onAddTaskForKr, onEdit }) {
    const [expanded, setExpanded] = useState(defaultExpanded);
...
// ─── Page principale ───────────────────────────────────────
export default function OKRIndex({ objectifs, filters, collaborateurs, periodes = [], axes = [], seuils = [], typesObjectifs = [], typesResultatsCles = [], configuration, vueOkr = 'cards', progressionGlobale = 0, velocite = 0, defaultCollaborateurId }) {
    ...
    return (
        <AppLayout title="Objectifs (OKR)">
            ...
            {periodes.length > 0 && (
                <div className="flex items-center gap-4 mb-5 overflow-x-auto border-b border-gray-200 dark:border-dark-800 max-w-5xl mx-auto">
                    {periodes.map(p => {
            ...
            {/* ═══ Filter Bar ═══ */}
            <div className="flex items-center gap-2 mb-5 flex-wrap max-w-5xl mx-auto">
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        className="pl-8 pr-3 py-1.5 bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-primary-500/30 focus:border-primary-500 w-40 transition-all"
                    />
                </div>
                <select
                    value={collabFilter}
                    onChange={(e) => { setCollabFilter(e.target.value); applyFilters('collaborateur_id', e.target.value); }}
                    className="bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700 rounded-lg text-xs px-2.5 py-1.5 pr-7 text-gray-600 dark:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500/30 focus:border-primary-500 cursor-pointer"
                >
            ...
            <div className="flex items-center justify-between mb-5 gap-4 max-w-5xl mx-auto">
