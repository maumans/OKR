...
import {
    ...
    Gauge, Award, Plus, Pencil, Trash2, Save, ArrowLeft, AlertTriangle
} from 'lucide-react';
...
// ─── Page principale ─────────────────────────────────────
export default function SuperAdminParametresOKR({
    societe, axes, periodes, typesObjectifs, typesResultatsCles,
    statuts, seuils, configuration, configurationPrime
}) {
    ...
    const handleDelete = (routeName, id) => {
        ...
        router.delete(route(routeName, [societe.id, id]));
    };

    // ─── Submit helpers ──────────────────────────────────
    const submitAxe = (e) => {
        e.preventDefault();
        const action = editingItem ? () => axeForm.put(route('superadmin.societes.parametres.axes.update', [societe.id, editingItem.id]), { onSuccess: () => setActiveDialog(null) })
            : () => axeForm.post(route('superadmin.societes.parametres.axes.store', societe.id), { onSuccess: () => setActiveDialog(null) });
        action();
    };
    const submitPeriode = (e) => {
        e.preventDefault();
        const action = editingItem ? () => periodeForm.put(route('superadmin.societes.parametres.periodes.update', [societe.id, editingItem.id]), { onSuccess: () => setActiveDialog(null) })
            : () => periodeForm.post(route('superadmin.societes.parametres.periodes.store', societe.id), { onSuccess: () => setActiveDialog(null) });
        action();
    };
    const submitTypeObj = (e) => {
        e.preventDefault();
        const action = editingItem ? () => typeObjForm.put(route('superadmin.societes.parametres.types-objectifs.update', [societe.id, editingItem.id]), { onSuccess: () => setActiveDialog(null) })
            : () => typeObjForm.post(route('superadmin.societes.parametres.types-objectifs.store', societe.id), { onSuccess: () => setActiveDialog(null) });
        action();
    };
    const submitTypeKR = (e) => {
        e.preventDefault();
        const action = editingItem ? () => typeKRForm.put(route('superadmin.societes.parametres.types-resultats.update', [societe.id, editingItem.id]), { onSuccess: () => setActiveDialog(null) })
            : () => typeKRForm.post(route('superadmin.societes.parametres.types-resultats.store', societe.id), { onSuccess: () => setActiveDialog(null) });
        action();
    };
    const submitStatut = (e) => {
        e.preventDefault();
        const action = editingItem ? () => statutForm.put(route('superadmin.societes.parametres.statuts.update', [societe.id, editingItem.id]), { onSuccess: () => setActiveDialog(null) })
            : () => statutForm.post(route('superadmin.societes.parametres.statuts.store', societe.id), { onSuccess: () => setActiveDialog(null) });
        action();
    };
    const submitSeuil = (e) => {
        e.preventDefault();
        const action = editingItem ? () => seuilForm.put(route('superadmin.societes.parametres.seuils.update', [societe.id, editingItem.id]), { onSuccess: () => setActiveDialog(null) })
            : () => seuilForm.post(route('superadmin.societes.parametres.seuils.store', societe.id), { onSuccess: () => setActiveDialog(null) });
        action();
    };

    return (
        <AppLayout header={`Paramètres OKR : ${societe.nom}`}>
            <div className="mb-6">
                <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 mb-1.5 rounded-full bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 text-[10px] font-semibold uppercase tracking-wider">
                            <Settings2 className="h-3.5 w-3.5" />
                            Configuration Système
                        </div>
                        <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                            Paramètres de {societe.nom}
                        </h1>
                        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Gérez les configurations fondamentales pour ce locataire.</p>
                    </div>
                </motion.div>
            </div>

            <div className="mb-6 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 rounded-xl p-4 flex gap-3 shadow-sm">
                <AlertTriangle className="h-5 w-5 text-amber-600 dark:text-amber-500 shrink-0 mt-0.5" />
                <div>
                    <h3 className="text-sm font-semibold text-amber-900 dark:text-amber-400">Attention : Modification en direct</h3>
                    <p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                        Les changements effectués sur ces onglets sont appliqués instantanément à l'environnement de <strong>{societe.nom}</strong>. 
                        Toute suppression d'un élément (ex: Axe stratégique, Période) utilisé par les collaborateurs entraînera des comportements inattendus côté client.
                    </p>
                </div>
            </div>
            ...
            <Tabs defaultValue="axes">
                ...
                <TabsContent value="axes">
                    <CrudSection
                        ...
                        onDelete={(item) => handleDelete('superadmin.societes.parametres.axes.destroy', item.id)}
                    />
                </TabsContent>
                ...
                <TabsContent value="periodes">
                    <CrudSection
                        ...
                        onDelete={(item) => handleDelete('superadmin.societes.parametres.periodes.destroy', item.id)}
                    />
                </TabsContent>
                ...
                <TabsContent value="types">
                    <CrudSection
                        ...
                        onDelete={(item) => handleDelete('superadmin.societes.parametres.types-objectifs.destroy', item.id)}
                    />
                </TabsContent>
                ...
                <TabsContent value="resultats">
                    <CrudSection
                        ...
                        onDelete={(item) => handleDelete('superadmin.societes.parametres.types-resultats.destroy', item.id)}
                    />
                </TabsContent>
                ...
                <TabsContent value="statuts">
                    <CrudSection
                        ...
                        onDelete={(item) => handleDelete('superadmin.societes.parametres.statuts.destroy', item.id)}
                    />
                </TabsContent>
                ...
                <TabsContent value="seuils">
                    <CrudSection
                        ...
                        onDelete={(item) => handleDelete('superadmin.societes.parametres.seuils.destroy', item.id)}
                    />
                </TabsContent>
                ...
                <TabsContent value="config">
                    <Card>
                        ...
                        <CardContent>
                            <form onSubmit={(e) => { e.preventDefault(); configForm.put(route('superadmin.societes.parametres.configuration.update', societe.id)); }} className="space-y-5 max-w-lg">
                                <div>
                ...
                <TabsContent value="primes">
                    <Card>
                        ...
                        <CardContent>
                            <form onSubmit={(e) => { e.preventDefault(); primeForm.put(route('superadmin.societes.parametres.primes.update', societe.id)); }} className="space-y-5 max-w-lg">
                                <div className="flex items-center gap-2">
